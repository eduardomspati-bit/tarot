...
if (MONGO_URI) {
    ...
    console.warn('⚠️ MONGO_URI no configurada.');
}
...
console.log('   API_KEY primeros 10 chars:', API_KEY ? API_KEY.substring(0, 10) + '...' : 'NO CONFIGURADA');
...
Devuelve HTML con class reading-section.`;
        ...
        console.log('   📤 Respuesta completa de Groq:', JSON.stringify(data, null, 2));

        if (!response.ok || !data.choices || data.choices.length === 0) {
            console.error('   ❌ Groq devolvió error:', data.error?.message || 'Sin choices');
            return res.status(500).json({
                error: 'Respuesta incompleta del proveedor de IA.',
                detalle: data.error?.message || `HTTP ${response.status} - Error desconocido`,
        ...
        if (!text) {
            console.warn('   ⚠️ Texto vacío después de limpiar, usando fallback');
            text = esModoGratis 
        ...
        console.error('   ❌ ERROR CATCH en /tirada:', error.message);
